...
export default function IncomeTaxClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-1">Income Tax Calculator 2025-26</h1>
      <div className="mb-2"><LastUpdatedBadge /></div>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">Compare Old vs New tax regime for FY 2025-26 and find which saves you more.</p>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            <h2 className="text-base font-semibold text-gray-800 dark:text-gray-200 mb-4">Income & Deductions</h2>
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-2">HRA Exemption (₹/year)</label>
              <input type="number" value={hraExemption} onChange={(e) => setHraExemption(Number(e.target.value))}
                className="w-40 text-sm font-semibold text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-700/50 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400 bg-brand-50 dark:bg-brand-900/20" />
            </div>
            <div className="mb-4">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-2">Other Deductions (80G, LTA, etc.) (₹)</label>
          ...
          {result && (
            <div id="tax-pdf-export" className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-amber-200 dark:border-amber-700/50 p-4">
                  <p className="text-xs font-semibold text-amber-600 uppercase mb-1">Old Regime</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{formatINR(result.taxOldRegime)}</p>
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-brand-200 dark:border-brand-700/50 p-4">
                  <p className="text-xs font-semibold text-brand-600 uppercase mb-1">New Regime</p>
                  <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{formatINR(result.taxNewRegime)}</p>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <h2 className="text-base font-semibold text-gray-800 dark:text-gray-200">Tax Slab Reference</h2>
              <span className="text-xs font-medium px-2.5 py-1 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/50 text-emerald-700 dark:text-emerald-400 rounded-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* Old Regime */}
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400 mb-2">Old Regime</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-800/60">
                      <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 rounded-tl-lg">Income Slab</th>
                      <th className="px-3 py-2 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 rounded-tr-lg">Rate</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={slab} className={`border-t border-gray-50 dark:border-gray-800 ${i % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50/50 dark:bg-gray-800/30"}`}>
                        <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{slab}</td>
                        <td className="px-3 py-2 text-right font-semibold text-amber-600 dark:text-amber-400">{rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">+ 4% cess · 87A rebate if income ≤ ₹5L · Std deduction ₹50,000</p>
              </div>

              {/* New Regime */}
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-brand-600 dark:text-brand-400 mb-2">New Regime</p>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-800/60">
                      <th className="px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 rounded-tl-lg">Income Slab</th>
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={slab} className={`border-t border-gray-50 dark:border-gray-800 ${i % 2 === 0 ? "bg-white dark:bg-gray-900" : "bg-gray-50/50 dark:bg-gray-800/30"}`}>
                        <td className="px-3 py-2 text-gray-700 dark:text-gray-300">{slab}</td>
                        <td className="px-3 py-2 text-right font-semibold text-brand-600 dark:text-brand-400">{rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">+ 4% cess · 87A rebate if income ≤ ₹12L · Std deduction ₹75,000</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            ...
            <div className="space-y-4 text-sm">
              <div><h3 className="font-semibold text-gray-800 dark:text-gray-100">What is the difference between Old and New tax regime?</h3><p className="text-gray-500 dark:text-gray-400 mt-1">The old regime allows deductions under 80C, 80D, HRA, etc. The new regime has lower tax rates but no deductions. If your deductions are large, old regime saves more; otherwise, new regime is better.</p></div>
              <div><h3 className="font-semibold text-gray-800 dark:text-gray-100">Who should choose the New Regime in FY 2025-26?</h3><p className="text-gray-500 dark:text-gray-400 mt-1">Taxpayers with income up to ₹12 lakh pay zero tax under the new regime due to the rebate under section 87A (FY 2025-26). Those with minimal deductions generally benefit more from the new regime.</p></div>
              <div><h3 className="font-semibold text-gray-800 dark:text-gray-100">What is included in Section 80C?</h3><p className="text-gray-500 dark:text-gray-400 mt-1">80C includes: EPF contributions, PPF, ELSS mutual funds, life insurance premiums, NSC, 5-year FD, and home loan principal repayment. Limit: ₹1.5 lakh.</p></div>
            </div>
          </div>
        </div>
