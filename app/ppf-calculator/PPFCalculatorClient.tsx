...
export default function PPFCalculatorClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-1">PPF Calculator</h1>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">Public Provident Fund — calculate maturity amount and year-by-year interest at current rate of 7.1% p.a.</p>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-6">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
            ...
            <div className="mb-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-2">PPF Interest Rate (% p.a.)</label>
              <input type="number" value={rate} onChange={(e) => setRate(parseFloat(e.target.value) || 7.1)} step={0.1} min={1} max={15}
                className="w-32 text-sm font-semibold text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-700/50 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400 bg-brand-50 dark:bg-brand-900/20" />
